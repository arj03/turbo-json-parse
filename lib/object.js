...
function defaults (opts) {
  ...
  class Required {
    ...
    validate () {
      ...
      if (this.count === 1) {
        ...
        const vs = this.vars.map((v, i) => `${v} !== ${this.mask(i)}`).join(' || ')
        this.gen(`if (${vs}) {`)
      }
  ...
  function compileObject (gen, prop, rawSchema, genany) {
    ...
    const allowEmpty = !fields.some(isRequired) &&
    ...
    if (!fields.length) {
      ...
      if (prettyPrinted) {
        gen(`if (${ch('ptr')} === ${code(' ')} || ${ch('ptr')} === ${code('\n')}) continue`)
      }
